...
### أ) تطبيق الويب — ده اللي تعرضيه في المناقشة

```bash
python scripts/run_app.py
```

وبعدين افتحي **http://localhost:8000**. تلات تبويبات:

| التبويب | بيعمل إيه |
|---|---|
| **Camera** | تتمرني قدام الكاميرا · عدّاد حي + حكم على كل عدة + تقرير |
| **YouTube link** | تحطي لينك · محتاج `pip install yt-dlp` (التبويب بيقولك لو مش متثبت) |
| **Upload a video** | ترفعي فيديو من الجهاز · بيرجّع فيديو معلّم عليه + تقرير JSON |

التلاتة بيشتغلوا على نفس `CoachEngine` اللي `run_live.py` بيستخدمه — يعني العدة اللي
بتتحسب في المتصفح بيحسبها نفس الكود اللي طلّع الأرقام الـ offline. الكاميرا بتشتغل
**على السيرفر** مش في المتصفح (المتصفح بيبعت فريمات ويستقبل نقاط) عشان العرض ما يبقاش
تطبيق تاني مختلف.

⚠️ **الكاميرا في المتصفح محتاجة `localhost` أو HTTPS.** لو فتحتي من الموبايل على عنوان
شبكة عادي، المتصفح هيرفض الكاميرا — صوّري على الموبايل واستخدمي تبويب **Upload**.

كل نتيجة بتيجي معاها لوحة **triage** (نسبة الكشف · زاوية الكاميرا · ثبات الكاميرا). لو
الزاوية مش جانبية، التطبيق **بيقول إن فحص تقوّس الضهر مش موثوق** بدل ما يدّي حكم مايقدرش
يدعمه. ده مقصود — وهو أهم سطر تقوليه لو حد سأل عن الحدود.

### ب) نافذة OpenCV (الطريقة القديمة)
